        ...
        inputSchema: {
          ...
          properties: {
            library: {
              ...
              description:
                'Which library to list components from. Defaults to "all".'
            },
            hasStory: {
              type: 'boolean',
              description:
        ...
        inputSchema: {
          ...
          properties: {
            ...
            includeVariants: {
              type: 'boolean',
              description:
                'Whether to generate variant stories (e.g., sizes, colors). Defaults to true.'
            },
            includeInteractive: {
              type: 'boolean',
              description:
                'Whether to include interactive stories with state. Defaults to false.'
            }
          },
          required: ['componentPath']
        }
        ...
        inputSchema: {
          ...
          properties: {
            ...
            library: {
              ...
              description:
                'Which library style to use. portal-ui includes MSW and router decorators.'
            }
          },
...
async function handleAnalyzeComponent(args: { componentPath: string }) {
  ...
  const analysis = {
    ...
    usesChakra:
      content.includes('@chakra-ui') || content.includes('@redesignhealth/ui'),
    usesRouter:
...
async function handleGenerateStory(args: {
  ...
  const storyTitle =
    args.storyTitle ||
    generateStoryTitle(componentName, library, args.componentPath)
  ...
  if (args.storyPath.includes('portal/ui')) {
    if (content.includes('useNavigate') || content.includes('Link')) {
      if (!content.includes('withRouter')) {
        issues.push(
          'Component uses React Router but story missing withRouter decorator'
        )
      }
    }
  }
...
function generateSuggestions(
  ...
  if (library === 'portal-ui') {
    suggestions.push(
      'Portal UI component - remember to include withRouter decorator if using routing'
    )
  }
