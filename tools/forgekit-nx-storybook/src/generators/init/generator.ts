  ...
  if (!nxStorybookReady) {
    ...
    if (shouldInstall) {
      ...
      try {
        ...
        ui.error(
          'Failed to install @nx/storybook. Please install it manually:'
        );
        ui.hint(
          `Run: ${ui.command('npm install --save-dev @nx/storybook --legacy-peer-deps')}`
        );
        return;
      }
      ...
      if (projectName) {
        ...
        try {
          ...
          ui.hint(
            `Run: ${ui.command(`npx nx g @nx/storybook:configuration --project=${projectName}`)}`
          );
        }
      }
    } else {
      ...
      ui.hint(
        `${ui.command('npm install --save-dev @nx/storybook && npx nx g @nx/storybook:configuration --project=<your-project>')}`
      );
    }
  } else {
...
async function promptConfigureProject(
  tree: Tree
): Promise<string | null> {
  ...
  try {
    const response = await prompt<{ project: string }>({
      ...
      name: 'project',
      message:
        'Which project should Storybook be configured for?',
      choices: [
        ...candidates,
