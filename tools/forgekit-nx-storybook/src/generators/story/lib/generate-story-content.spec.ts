...
import { generateStoryContent, StoryContentOptions } from './generate-story-content';
...
function makeOptions(overrides: Partial<StoryContentOptions> = {}): StoryContentOptions {
  return {
...
describe('generateStoryContent', () => {
      ...
      makeOptions({
        analysis: makeAnalysis({
          props: [
            ...
            { name: 'label', type: 'string', required: false, isCallback: false },
            { name: 'count', type: 'number', required: false, isCallback: false },
          ],
        }),
      })
    );
      ...
      makeOptions({
        analysis: makeAnalysis({
          props: [
            ...
            { name: 'count', type: 'number', required: true, isCallback: false },
            { name: 'onClick', type: '() => void', required: true, isCallback: true },
          ],
        }),
      })
    );
  ...
  it('should not generate default args for optional props', () => {
    const content = generateStoryContent(
      makeOptions({
        analysis: makeAnalysis({
          props: [
            { name: 'subtitle', type: 'string', required: false, isCallback: false },
          ],
        }),
      })
    );
  ...
  it('should add router decorator when component uses router', () => {
    ...
    expect(content).toContain("import { withRouter } from 'storybook-addon-react-router-v6'");
    ...
    const content = generateStoryContent(
      makeOptions({
        analysis: makeAnalysis({
          ...
          props: [
            { name: 'children', type: 'React.ReactNode', required: false, isCallback: false },
            { name: 'title', type: 'string', required: false, isCallback: false },
          ],
        }),
      })
    );
