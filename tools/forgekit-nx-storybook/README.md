...
`@effinrich/forgekit-nx-storybook` handles the **content** — analyzing your React components and generating ready-to-use `.stories.tsx` and `.ct.tsx` files with controls, variants, interaction tests, and accessibility checks.

**Use both together.** This plugin requires `@nx/storybook` as a peer dependency and delegates all config scaffolding to it.

| Concern                                           | @nx/storybook | @effinrich/forgekit-nx-storybook |
| ------------------------------------------------- | :-----------: | :------------------------------: |
...
npx nx add @effinrich/forgekit-nx-storybook
```
...
```bash
npx nx g @effinrich/forgekit-nx-storybook:story --componentPath=libs/shared/ui/src/lib/button/button.tsx
```
...
```bash
npx nx g @effinrich/forgekit-nx-storybook:stories --project=shared-ui
```
...
```bash
npx nx g @effinrich/forgekit-nx-storybook:component-test --componentPath=libs/shared/ui/src/lib/button/button.tsx
```
...
```bash
npx nx g @effinrich/forgekit-nx-storybook:init
```
...
```json
{
  "targets": {
    "watch-stories": {
      "executor": "@effinrich/forgekit-nx-storybook:watch",
      "options": {
...
} from '@effinrich/forgekit-nx-storybook'
