...
export class AddUserForm {
  ...
  constructor(page: Page) {
    ...
    this.adminRB = this.createUserRoleRB(/^Admin/)
    this.rhUserRB = this.createUserRoleRB('RH User')
    this.coUserRB = this.createUserRoleRB('Company User')
  ...
  private createUserRoleRB(role) {
    return new ChakraRadioButton(
      this.modal.locator('.chakra-radio', { hasText: role })
    )
  }
  async addUser(input) {
  ...
  async fillout(input) {
    if (input.userType) {
      if (input.userType.match(/Super Admin/i)) {
        await this.superAdminRB.click()
      } else if (input.userType.match(/Admin/i)) {
        await this.adminRB.click()
      } else if (input.userType.match(/RH User/i)) {
        await this.rhUserRB.click()
      } else if (input.userType.match(/Company User/i)) {
        await this.coUserRB.click()
      } else {
        throw new Error(`Unknown User Type:  ${input.userType}`)
      }
    }
    if (input.email) {
    ...
    if (input.company) {
      // type the value instead of selecting from list
