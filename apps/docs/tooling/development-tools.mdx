...
The `forgekit-nx-storybook` plugin lives in `tools/forgekit-nx-storybook/` and provides generators and an executor for automated Storybook story generation.

### Generators

<Tabs>
  <Tab title="init">
    Runs when you install the plugin via `nx add forgekit-nx-storybook`. It checks for `@nx/storybook`, offers to install it if missing, ensures Storybook core dependencies are present, and prints available commands.
...
sequenceDiagram
    ...
    Dev->>Gen: nx g forgekit-nx-storybook:story --componentPath=...
...
A Model Context Protocol server at `tools/storybook-mcp/` integrates Storybook component generation with AI assistants:

```bash
npm run mcp:setup        # First-time install + build
